...
const WomensDayGallery = () => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        setLoading(true);
        const res = await api.get('/womensday/submissions');
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Women's Day Gallery</h1>
      ...
      {!loading && !error && filtered.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((s) => (
            <Card
              key={s.id}
              className="overflow-hidden cursor-pointer hover:shadow-lg transition-shadow border border-pink-100 group"
            ...
            >
              ...
              <CardContent className="p-4 space-y-2">
                ...
                <p className="text-xs text-gray-500 line-clamp-1">
                  <span className="font-medium text-pink-600">Interests:</span> {s.interests}
                </p>
                <p className="text-xs text-gray-500 line-clamp-1">
                  <span className="font-medium text-purple-600">Dream:</span> {s.dream}
                </p>
              </CardContent>
            </Card>
          ))}
        ...
        <Dialog open onOpenChange={() => setSelected(null)}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {/* Portrait */}
              <div className="rounded-xl overflow-hidden bg-gradient-to-br from-pink-50 to-purple-50">
                <img
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-pink-50 rounded-lg p-3">
                  <p className="text-xs font-semibold text-pink-600 uppercase tracking-wide mb-1">Interests</p>
                  <p className="text-sm text-gray-800">{selected.interests}</p>
                </div>
                <div className="bg-purple-50 rounded-lg p-3">
                  <p className="text-xs font-semibold text-purple-600 uppercase tracking-wide mb-1">Dream</p>
              ...
              <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg p-4 border border-pink-100">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">AI Compliment (Darija)</p>
